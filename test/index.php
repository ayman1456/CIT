<?php
session_start();

;?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">
</head>
<body>
<!--navbar starts-->
<nav class="navbar navbar-expand-lg bg-body-tertiary">
  <div class="container-fluid">
    <a class="navbar-brand" href="#">Navbar</a>
    <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
      <span class="navbar-toggler-icon"></span>
    </button>
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      <ul class="navbar-nav mx-auto mb-2 mb-lg-0">
        <li class="nav-item">
          <a class="nav-link active" aria-current="page" href="#">Add Post</a>
        </li>
        <li class="nav-item">
          <a class="nav-link active" aria-current="page" href="#">All Post</a>
        </li>
      
    </div>
  </div>
</nav>

<!--navbar end-->
<div class="card col-lg-4 mx-auto mt-5">
  <div class="card-header"><div class="mb-3">
    <p> ADD POST<p>

</div>
</div>
  <div class="card-body">
  <div class="mb-3">
  <form action="./controller/store.php" method="POST">

  <div class="mb-3">
  <input type="text" value="<?=$_SESSION['title']??""?>"name="title"class="form-control"  placeholder="Title">
  <span class="text-danger">
  <?=$_SESSION['errors']['title']??""?>
</span>
 
</div>
<div class="mb-3">
  <textarea class="form-control" name="detail" placeholder="Details" rows="3"><?=$_SESSION['detail']??""?></textarea>
  <span class="text-danger">
  <?=$_SESSION['errors']['detail']??""?>
  </span>
</div>

  <button class="btn btn-primary w-100">Submit</button>
</form>
</div>
  </div>

</div>


<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz" crossorigin="anonymous"></script>
</body>
</html>

<?php
session_destroy();
?>